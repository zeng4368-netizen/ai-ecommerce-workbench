(function(root){
 ...
 async function mount(node,{toast,masked}){
  ...
  function controls(){const busy=isSending||current?.busy,archived=current?.deleted;input.disabled=!!archived;$('chatSend').disabled=!!busy||!!archived;$('chatStop').hidden=!busy;$('chatWorkspace').disabled=!!busy||!!archived;$('chatRename').disabled=!current||!!busy;$('chatExport').disabled=!current;$('chatDelete').disabled=!current||!!busy;$('chatDelete').textContent=archived?'恢复':'删除';}
  ...
  function render(scroll=true){
   $('chatTitle').textContent=current?.title||'新对话';$('chatWorkspace').checked=current?.workspace_enabled??true;
   const activeVersion=current?.pinned_version||DataHub.current?.version;
   $('chatVersion').textContent=activeVersion?(current?.pinned_version?'报告固定版本 ':'版本 ')+activeVersion.slice(0,8):'';
  ...
  async function send(retry){
   ...
   try{
    if(!current){current=await api('/conversations',json({workspace_enabled:$('chatWorkspace').checked}));if(!valid()||sendingView!==selectRequest)return;setSelected(current.id);}
    ...
    const response=await fetch('/api/hub/assistant/stream',{...json({question,conversation_id:id,request_id:requestId,expected_revision:expected,workspace_enabled:$('chatWorkspace').checked,version:DataHub.current?.version||''}),signal:controller.signal});
  ...
  $('chatWorkspace').onchange=capture(async()=>{if(!current)return;current=await api('/conversations/'+current.id,json({expected_revision:current.revision,workspace_enabled:$('chatWorkspace').checked},'PATCH'));if(!valid())return;controls();});
  ...
  try{const health=await (await fetch('/api/health')).json();if(!valid())return;$('chatModel').textContent=health.llm==='configured'?(health.model||'DeepSeek')+' · '+(health.provider||'已配置模型'):'模型尚未配置';const items=await list();if(!valid())return;if(jump){jump=false;await open(selected);}else if(selected&&items.some(c=>c.id===selected))await open(selected);else if(seed)fresh(false);else if(items.length)await open(items[0].id);else fresh(false);}catch(error){if(valid())$('chatStatus').textContent=error.message;}
 }
